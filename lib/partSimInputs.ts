...
import type { DemoPartId, PartId } from '@/lib/types'
 ...
 *  preset request for the four demo parts, or null for any other ID
 *  ('custom' and user-part IDs build their FullAnalysisRequest on the
 *  fly instead). */
export function getPartSimInputs(id: PartId): FullAnalysisRequest | null {
  return partSimInputs[id as DemoPartId] ?? null
}

export const partSimInputs: Record<DemoPartId, FullAnalysisRequest> = {
  bracket: {
    material: 'ABS',
    wall_thickness: 2.5,
    part_volume: 48,
